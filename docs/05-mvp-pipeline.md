...
- **MVP 公式本身**只由 World / View / Projection 参数决定
- **最终画面的视觉反馈**还会受到模型顶点分布和几何轮廓影响

这意味着：把立方体切换成球体或圆柱，并不会改变 `MVP = M_P × M_V × M_W` 这条公式本身；改变的是“同一套矩阵作用到哪一组顶点上”，因此你看到的旋转感、轮廓变化和透视体感会不同。
